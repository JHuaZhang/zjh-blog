...
可以使用 ls -l 命令查看文件的权限，显示文件的详细信息，包括权限、所有者、组、大小等。
...
在 Linux 中，文件或目录的权限通过一串符号表示，例如-rw-r--r--。这些符号分为文件类型和权限细节两部分，具体含义如下：
...
后 9 个字符分为 3 组，每组 3 个字符，依次表示以下权限：

- 所有者（Owner）权限（第 2-4 位）。
- 所属组（Group）权限（第 5-7 位）。
- 其他用户（Others）权限（第 8-10 位）。
...
| 符号 | 名称   | 作用                                 | 示例       |
| ---- | ------ | ------------------------------------ | ---------- |
| s    | SetUID | 以文件所有者身份执行（如 passwd）    | -rwsr-xr-x |
